  ...
  "status": "PAID"
    ...
    function shortLine(value, fallback) {
      const text = String(value || '').trim();
      if (!text) return fallback || 'Not set';
    ...
    function updateHuntDashboard() {
      ...
      const metaEl = document.getElementById('huntSessionMeta');
      ...
      if (metaEl) {
        const labels = { bug_bounty: 'Bug Bounty', vdp: 'VDP', internal: 'Internal / Pentest' };
        metaEl.innerHTML = [
          '<span>' + escapeHTML(labels[session.program] || session.program || 'Program not set') + '</span>',
          '<span>Scope: ' + escapeHTML(shortLine(session.scope, 'Not set')) + '</span>',
          '<span>Limits: ' + escapeHTML(shortLine(session.outOfScope, 'Not set')) + '</span>'
        ].join('');
    ...
    function openStartHuntModal() {
      ...
      const scopeInp = document.getElementById('shuntScopeInput');
      const outScopeInp = document.getElementById('shuntOutScopeInput');
      const notesInp = document.getElementById('shuntNotesInput');
      const authCheck = document.getElementById('shuntAuthCheck');
      const session = JSON.parse(localStorage.getItem('bb_hunt_session') || 'null');
      if (inp) {
        inp.value = session ? session.target : '';
        setTimeout(function() { inp.focus(); }, 100);
      }
      if (scopeInp) scopeInp.value = session ? (session.scope || '') : '';
      if (outScopeInp) outScopeInp.value = session ? (session.outOfScope || '') : '';
      if (notesInp) notesInp.value = session ? (session.notes || '') : '';
    ...
    function selectProgram(type, el) {
      ...
      if (!authCheck || !authCheck.checked) {
        const label = authCheck ? authCheck.closest('.shunt-check') : null;
        if (label) {
          label.style.color = '#ffb020';
      ...
      setTimeout(function() {
        const target = (document.getElementById('shuntTargetInput') || {}).value || '';
        const scope = (document.getElementById('shuntScopeInput') || {}).value || '';
        const outScope = (document.getElementById('shuntOutScopeInput') || {}).value || '';
        const notes = (document.getElementById('shuntNotesInput') || {}).value || '';
    ...
    function showToolInfo(toolName, description) {
      // Simulate in terminal if on phase p19, otherwise show tooltip panel
      var isTermVisible = document.getElementById('meth-content-p19') &&
                          document.getElementById('meth-content-p19').style.display !== 'none';

      // Remove existing panel
      if (_toolInfoPanel) { _toolInfoPanel.remove(); _toolInfoPanel = null; }

      // Map tool to a simulated output
      var simOutputs = {
        subfinder: 'subfinder -d target.com -all -silent\n[info] Starting Subfinder v2.6.2...\n[info] Querying 24 passive sources...\napi.target.com\ndev.target.com        ← HOT 🎯\nstaging.target.com    ← HOT 🎯\nadmin.target.com      ← HOT 🎯\nassets.target.com\ncdn.target.com\n[+] Total: 6 subdomains found.',
        ...
        amass:     'amass enum -passive -d target.com\n[info] OWASP Amass v3.23.2...\napi.target.com          [Google: transparency.google.com]\ndev-portal.target.com   [CertSpotter]\nauth.target.com         [VirusTotal]\ncdn-assets.target.com   [DNSdumpster]\n[+] 4 subdomains (passive mode).',
        assetfinder: 'assetfinder --subs-only target.com\napi.target.com\nstatus.target.com\nblog.target.com\n[+] 3 subdomains via crt.sh / certspotter.'
      };

      var output = simOutputs[toolName] || (toolName + ' -h\n[+] Tool info: ' + description);
