window.quizzesData = {
    "rh124_cli": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لإنشاء اتصال آمن مشفر بجهاز سيرفر عن بعد؟",
                "options": [
                    "telnet",
                    "ssh",
                    "ftp",
                    "rlogin"
                ],
                "answer": 1,
                "explanation": "الأمر ssh (Secure Shell) يوفر اتصالاً آمناً ومشفراً، على عكس telnet الذي يرسل البيانات بنص واضح."
            }
        ],
        "medium": [
            {
                "q": "ما هو منفذ الاتصال الافتراضي (Default Port) لخدمة SSH؟",
                "options": [
                    "21",
                    "22",
                    "23",
                    "80"
                ],
                "answer": 1,
                "explanation": "يعمل بروتوكول SSH بشكل افتراضي على المنفذ رقم 22."
            }
        ],
        "hard": [
            {
                "q": "أي خيار في أمر ssh يسمح بتحديد منفذ مخصص للاتصال؟",
                "options": [
                    "-p",
                    "-P",
                    "-port",
                    "-s"
                ],
                "answer": 0,
                "explanation": "الخيار -p (حرف صغير) يستخدم لتحديد منفذ غير افتراضي، مثلاً: ssh -p 2222 user@host."
            }
        ],
        "scenario": [
            {
                "q": "كيف يمكن إنشاء هيكل مجلدات متداخلة /project/{docs,src,tests}/{2024,2025} بأمر واحد؟",
                "options": [
                    "mkdir -r /project/{docs,src,tests}/{2024,2025}",
                    "mkdir -p /project/{docs,src,tests}/{2024,2025}",
                    "mkdir -d /project/(docs,src,tests)/(2024,2025)",
                    "mkdir -m /project/[docs,src,tests]/[2024,2025]"
                ],
                "answer": 1,
                "explanation": "الأمر mkdir -p يقوم بإنشاء المجلدات الأبوية بشكل تكراري، وتُستخدم الأقواس المتعرجة {} لتوليد توليفات المسارات تلقائياً (Brace Expansion)."
            }
        ]
    },
    "rh124_files": {
        "easy": [
            {
                "q": "أي أمر يستخدم لمعرفة المجلد الحالي الذي تتواجد فيه الآن (Working Directory)؟",
                "options": [
                    "ls",
                    "cd",
                    "pwd",
                    "dir"
                ],
                "answer": 2,
                "explanation": "الأمر pwd يعني (Print Working Directory) ويعرض المسار الكامل للمجلد الحالي."
            },
            {
                "q": "أي مجلد قياسي في هيكلية FHS مخصص لتخزين ملفات الإعدادات والتهيئة الخاصة بالنظام والتطبيقات؟",
                "options": [
                    "/var",
                    "/usr",
                    "/etc",
                    "/boot"
                ],
                "answer": 2,
                "explanation": "المجلد /etc هو المجلد القياسي لحفظ ملفات التكوين والإعدادات في أنظمة لينكس."
            }
        ],
        "medium": [
            {
                "q": "كيف يمكنك الانتقال إلى المجلد الرئيسي للمستخدم الحالي (Home Directory) مباشرة؟",
                "options": [
                    "cd /",
                    "cd ..",
                    "cd ~",
                    "cd ."
                ],
                "answer": 2,
                "explanation": "الرمز ~ يرمز للمجلد الرئيسي للمستخدم الحالي (Home Directory)."
            }
        ],
        "hard": [
            {
                "q": "ما هو الفرق الرئيسي بين المسار المطلق (Absolute Path) والمسار النسبي (Relative Path)؟",
                "options": [
                    "المسار المطلق يبدأ دائماً من الجزر / بينما النسبي يبدأ من المجلد الحالي.",
                    "المسار النسبي يبدأ دائماً من الجزر / بينما المطلق يبدأ من المجلد الحالي.",
                    "المسار المطلق أسرع في التنفيذ دائماً.",
                    "لا يوجد فرق بينهما في الاستخدام الفعلي."
                ],
                "answer": 0,
                "explanation": "المسار المطلق يحدد الطريق من جذر النظام / بالكامل، بينما النسبي يعتمد على موقعك الحالي."
            }
        ]
    },
    "rh124_help": {
        "easy": [
            {
                "q": "ما هو الأمر الأساسي لعرض دليل المساعدة والكتالوج لأي أمر في لينكس؟",
                "options": [
                    "help",
                    "man",
                    "info",
                    "whatis"
                ],
                "answer": 1,
                "explanation": "الأمر man (Manual) يعرض صفحات المساعدة المفصلة لأوامر النظام."
            }
        ],
        "medium": [
            {
                "q": "في صفحات المان (man pages)، أي قسم مخصص لعرض ملفات التكوين والإعدادات (Configuration files)؟",
                "options": [
                    "القسم 1",
                    "القسم 5",
                    "القسم 8",
                    "القسم 2"
                ],
                "answer": 1,
                "explanation": "القسم 5 في صفحات المان يحتوي على شروحات وتنسيقات ملفات التكوين والإعدادات مثل /etc/passwd."
            }
        ],
        "hard": [
            {
                "q": "كيف يمكنك البحث عن كلمة مفتاحية داخل جميع عناوين صفحات المساعدة (man pages)؟",
                "options": [
                    "man -k [keyword]",
                    "man -f [keyword]",
                    "man -s [keyword]",
                    "man -g [keyword]"
                ],
                "answer": 0,
                "explanation": "الخيار -k (أو الأمر apropos) يبحث عن الكلمة المفتاحية في ملخصات وعناوين صفحات المان."
            }
        ]
    },
    "rh124_redirection": {
        "easy": [
            {
                "q": "أي رمز يستخدم لإضافة المخرجات إلى نهاية ملف قائم دون مسح محتوياته؟",
                "options": [
                    ">",
                    ">>",
                    "<",
                    "|"
                ],
                "answer": 1,
                "explanation": "الرمز >> (Append) يضيف المخرجات إلى نهاية الملف، بينما > يمسح المحتوى القديم ويكتب من جديد."
            }
        ],
        "medium": [
            {
                "q": "كيف يمكنك توجيه رسائل الخطأ فقط (Standard Error) إلى ملف محدد لتجنب ظهورها على الشاشة؟",
                "options": [
                    "> file.log",
                    "1> file.log",
                    "2> file.log",
                    "stderr> file.log"
                ],
                "answer": 2,
                "explanation": "الرقم 2 يمثل الواصف القياسي للخطأ (File Descriptor 2)، وتوجيهه يكون بـ 2>."
            }
        ],
        "hard": [
            {
                "q": "ما هو دور الأنبوب (Pipe `|`) في لينكس؟",
                "options": [
                    "توجيه مخرجات أمر لتعمل كمدخلات للأمر الذي يليه.",
                    "حفظ الملفات في الخلفية.",
                    "إعطاء صلاحيات للمستخدمين.",
                    "قياس سرعة الاتصال بالشبكة."
                ],
                "answer": 0,
                "explanation": "يقوم الأنبوب | بربط مخرجات الأمر الأول (stdout) ليعمل مباشرة كمدخلات للأمر الثاني (stdin)."
            }
        ]
    },
    "rh124_users": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لإنشاء مستخدم محلي جديد في النظام؟",
                "options": [
                    "adduser",
                    "useradd",
                    "newuser",
                    "createuser"
                ],
                "answer": 1,
                "explanation": "الأمر الأساسي في توزيعات Red Hat لإنشاء مستخدم هو useradd."
            }
        ],
        "medium": [
            {
                "q": "أي ملف يحتوي على معلومات هوية المستخدمين، الـ UID، ومسار الشل الافتراضي لكل مستخدم؟",
                "options": [
                    "/etc/shadow",
                    "/etc/passwd",
                    "/etc/group",
                    "/etc/sudoers"
                ],
                "answer": 1,
                "explanation": "يحتوي ملف /etc/passwd على معلومات الحسابات ومجلداتهم وهوياتهم بنص واضح."
            },
            {
                "q": "ما هي سياسة المجموعات الخاصة بالمستخدمين (User Private Groups) الافتراضية في RHEL؟",
                "options": [
                    "أن تتم إضافة كل مستخدم جديد لمجموعة الـ root تلقائياً.",
                    "أن يتم إنشاء مجموعة جديدة فريدة لكل مستخدم تحمل نفس اسمه ويكون هو العضو الوحيد فيها.",
                    "أن يتم منع المستخدمين من تغيير صلاحيات مجلداتهم المنزلية.",
                    "أن يشترك جميع المستخدمين العاديين في مجموعة واحدة افتراضية تسمى users."
                ],
                "answer": 1,
                "explanation": "عند إنشاء مستخدم في RHEL، يتم إنشاء مجموعة خاصة به بنفس اسمه تماماً كإجراء أمني لتسهيل إدارة الصلاحيات لاحقاً."
            }
        ],
        "hard": [
            {
                "q": "أين يتم تخزين الهاش المشفر لكلمات مرور المستخدمين وتواريخ صلاحيتها؟",
                "options": [
                    "/etc/passwd",
                    "/etc/shadow",
                    "/etc/secure",
                    "/etc/gshadow"
                ],
                "answer": 1,
                "explanation": "يخزن ملف /etc/shadow الهاشات المشفرة لكلمات المرور ولا يمكن قراءته إلا بصلاحيات root لأسباب أمنية."
            }
        ]
    },
    "rh124_perms": {
        "easy": [
            {
                "q": "ما هي القيمة الرقمية المقابلة لصلاحيات القراءة والكتابة فقط (Read & Write) للمالك دون التشغيل؟",
                "options": [
                    "7",
                    "6",
                    "5",
                    "4"
                ],
                "answer": 1,
                "explanation": "القراءة (4) + الكتابة (2) = 6."
            }
        ],
        "medium": [
            {
                "q": "إذا كان الـ Umask الافتراضي هو 022، ما هي الصلاحيات الافتراضية التي سيحصل عليها أي مجلد جديد عند إنشائه؟",
                "options": [
                    "644",
                    "755",
                    "700",
                    "777"
                ],
                "answer": 1,
                "explanation": "المجلدات تحصل افتراضياً على صلاحية كاملة 777 وتطرح منها قيمة الـ Umask (777 - 022 = 755)."
            }
        ],
        "hard": [
            {
                "q": "كيف يمكن تغيير مالك ملف والمجموعة المالكة له في نفس الوقت باستخدام chown؟",
                "options": [
                    "chown user:group file.txt",
                    "chown user -g group file.txt",
                    "chown user file.txt && chgrp group file.txt",
                    "كل ما سبق صحيح ولكن الخيار الأول هو الأقصر والأفضل"
                ],
                "answer": 3,
                "explanation": "الأمر chown user:group file.txt يقوم بتغيير المالك والمجموعة دفعة واحدة وبشكل قياسي."
            }
        ]
    },
    "rh124_processes": {
        "easy": [
            {
                "q": "ما هو الأمر الذي يعرض العمليات النشطة حالياً في النظام بشكل شجري أو كجدول؟",
                "options": [
                    "kill",
                    "ps",
                    "top",
                    "systemctl"
                ],
                "answer": 1,
                "explanation": "الأمر ps (Process Status) يعرض حالة العمليات الحالية بالكامل."
            }
        ],
        "medium": [
            {
                "q": "أي إشارة (Signal) يتم إرسالها للعملية عند استخدام الأمر `kill -9`؟",
                "options": [
                    "SIGTERM",
                    "SIGINT",
                    "SIGKILL",
                    "SIGHUP"
                ],
                "answer": 2,
                "explanation": "الإشارة رقم 9 ترمز لـ SIGKILL والتي تجبر النواة على قتل العملية فوراً دون السماح لها بحفظ البيانات."
            }
        ],
        "hard": [
            {
                "q": "ما هو الفرق بين SIGTERM (إشارة 15) و SIGKILL (إشارة 9)؟",
                "options": [
                    "SIGTERM تطلب من العملية الإغلاق بشكل منظم وآمن، بينما SIGKILL تنهيها فوراً بقوة النواة.",
                    "SIGKILL تطلب من العملية الإغلاق بشكل منظم، بينما SIGTERM تنهيها بقوة.",
                    "SIGTERM تستخدم فقط مع الـ Root.",
                    "لا فرق بينهما في التأثير الفعلي."
                ],
                "answer": 0,
                "explanation": "الإشارة 15 (SIGTERM) هي الطريقة اللطيفة لإغلاق البرمجيات بشكل منظم، بينما 9 (SIGKILL) تنهيها قسرياً."
            }
        ]
    },
    "rh124_services": {
        "easy": [
            {
                "q": "ما هي الأداة القياسية في RHEL 9 لإدارة الخدمات وإيقافها وتشغيلها؟",
                "options": [
                    "service",
                    "systemctl",
                    "init",
                    "chkconfig"
                ],
                "answer": 1,
                "explanation": "الأداة systemctl هي الواجهة الرئيسية لنظام الـ Init الحديث systemd."
            }
        ],
        "medium": [
            {
                "q": "ما هو الفرق بين الخيارين start و enable عند تشغيل الخدمات بـ systemctl؟",
                "options": [
                    "start تشغل الخدمة حالياً، بينما enable تجعلها تعمل تلقائياً مع إقلاع النظام المستقبلي.",
                    "enable تشغل الخدمة حالياً، بينما start تجعلها تعمل مع الإقلاع المستقبلي.",
                    "لا يوجد فرق بينهما.",
                    "enable تتطلب كلمة مرور root دائماً بينما start لا تتطلب."
                ],
                "answer": 0,
                "explanation": "الخيار start ينفذ التشغيل اللحظي الفوري، بينما enable يضع الخدمة في قائمة الإقلاع التلقائي (Startup)."
            }
        ],
        "hard": [
            {
                "q": "كيف يمكنك التحقق مما إذا كانت خدمة معينة نشطة وتعمل الآن (Active) ومثبتة للعمل مع الإقلاع بطلب واحد؟",
                "options": [
                    "systemctl status [service]",
                    "systemctl is-active [service]",
                    "systemctl show [service]",
                    "systemctl inspect [service]"
                ],
                "answer": 0,
                "explanation": "الأمر systemctl status يعطي تقريراً كاملاً عن حالة الخدمة اللحظية وحالة إقلاعها وآخر السجلات الجنائية الخاصة بها."
            }
        ]
    },
    "rh124_ssh_security": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لتوليد مفاتيح تشفير SSH (عام وخاص) على جهازك؟",
                "options": [
                    "ssh-keygen",
                    "ssh-copy-id",
                    "ssh-add",
                    "crypto key"
                ],
                "answer": 0,
                "explanation": "الأمر ssh-keygen يولد زوج المفاتيح (المفتاح الخاص id_rsa والمفتاح العام id_rsa.pub) تلقائياً."
            }
        ],
        "medium": [
            {
                "q": "ما هو الأمر الصحيح والآمن لنقل مفتاحك العام إلى خادم بعيد لتفعيل تسجيل الدخول بدون باسورد؟",
                "options": [
                    "ssh-copy-id",
                    "scp id_rsa.pub",
                    "ssh-keygen",
                    "cat id_rsa.pub > server"
                ],
                "answer": 0,
                "explanation": "الأمر ssh-copy-id يقوم بنقل وتأمين المفتاح العام وإضافته لملف authorized_keys على السيرفر البعيد بشكل تلقائي وسليم أمنياً."
            }
        ],
        "hard": [
            {
                "q": "ما هو الملف الذي تخزن فيه المفاتيح العامة المصرح لها بالاتصال بالسيرفر على حساب المستخدم؟",
                "options": [
                    "~/.ssh/authorized_keys",
                    "~/.ssh/known_hosts",
                    "~/.ssh/id_rsa",
                    "/etc/ssh/sshd_config"
                ],
                "answer": 0,
                "explanation": "يحتوي الملف ~/.ssh/authorized_keys على المفاتيح العامة للأجهزة المصرح لها بالولوج بدون كلمة مرور."
            }
        ]
    },
    "rh124_logs": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم في توزيعات Red Hat لاستعراض سجلات الأحداث التي يديرها نظام systemd؟",
                "options": [
                    "cat /var/log/messages",
                    "journalctl",
                    "dmesg",
                    "syslog"
                ],
                "answer": 1,
                "explanation": "الأداة journalctl هي الأداة الرسمية لقراءة وفحص سجلات النظام المخزنة بـ systemd-journald."
            }
        ],
        "medium": [
            {
                "q": "كيف يمكنك متابعة سجلات الأحداث لخدمة معينة بشكل حي ومستمر (Real-time monitoring)؟",
                "options": [
                    "journalctl -u [service] -f",
                    "journalctl -u [service] -n 10",
                    "journalctl -u [service] -e",
                    "journalctl -u [service] -p err"
                ],
                "answer": 0,
                "explanation": "الخيار -f (Follow) يجعل المخرجات مستمرة وحية وتظهر على الشاشة فور وقوع الحدث."
            }
        ],
        "hard": [
            {
                "q": "أين تقع السجلات الجنائية المسؤولة عن تسجيل عمليات تسجيل الدخول الناجحة والفاشلة واستخدام sudo في RHEL 9؟",
                "options": [
                    "/var/log/secure",
                    "/var/log/auth.log",
                    "/var/log/messages",
                    "/var/log/audit/audit.log"
                ],
                "answer": 0,
                "explanation": "في عائلة Red Hat يتم تسجيل عمليات المصادقة والأمان في ملف /var/log/secure (بينما في Debian/Ubuntu تسمى auth.log)."
            }
        ]
    },
    "rh124_network": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لعرض عناوين الـ IP لجميع بطاقات الشبكة في نظام لينكس الحديث؟",
                "options": [
                    "ifconfig",
                    "ip a",
                    "netstat",
                    "route"
                ],
                "answer": 1,
                "explanation": "الأمر ip a (أو ip address) هو البديل الحديث والمعتمد لعرض وضبط الشبكات عوضاً عن ifconfig القديم."
            }
        ],
        "medium": [
            {
                "q": "ما هي الأداة المفتاحية في Red Hat لإدارة اتصالات الشبكة وبطاقاتها عبر سطر الأوامر؟",
                "options": [
                    "ifup",
                    "nmcli",
                    "network-control",
                    "ip route"
                ],
                "answer": 1,
                "explanation": "الأداة nmcli (NetworkManager CLI) هي الأداة الأساسية لإدارة الشبكات بالكامل في RHEL."
            }
        ],
        "hard": [
            {
                "q": "كيف يمكنك تفعيل تغييرات الشبكة التي أجريتها على بطاقة اتصال باستخدام nmcli؟",
                "options": [
                    "nmcli connection up [conn_name]",
                    "nmcli connection reload",
                    "systemctl restart network",
                    "nmcli connection apply"
                ],
                "answer": 0,
                "explanation": "لتطبيق التغييرات وتفعيل الاتصال نستخدم nmcli connection up متبوعاً باسم الاتصال."
            }
        ]
    },
    "rh124_archive": {
        "easy": [
            {
                "q": "ما هو الأمر الأساسي لإنشاء ملفات الأرشيف (تجميع الملفات) وضغطها؟",
                "options": [
                    "zip",
                    "tar",
                    "gzip",
                    "compress"
                ],
                "answer": 1,
                "explanation": "الأمر tar (Tape Archive) هو الأداة القياسية لأرشفة وضغط الملفات والمجلدات في لينكس."
            }
        ],
        "medium": [
            {
                "q": "أي خيارات يجب تمريرها للأمر tar لإنشاء ملف مضغوط بصيغة gzip وتسميته؟",
                "options": [
                    "-xvf",
                    "-czvf",
                    "-tjvf",
                    "-dzvf"
                ],
                "answer": 1,
                "explanation": "الخيار -c لإنشاء أرشيف (Create)، و -z للضغط بـ gzip، و -v لعرض التفاصيل (Verbose)، و -f لتحديد اسم الملف (File)."
            }
        ],
        "hard": [
            {
                "q": "ما هي الميزة الأساسية للأداة rsync مقارنة بـ scp عند نقل الملفات؟",
                "options": [
                    "تقوم بنقل الفروقات فقط (Delta Transfer) وتدعم استكمال النقل مما يوفر وقت الباندويدث.",
                    "تستخدم تشفيراً أقوى بكثير.",
                    "لا تتطلب وجود مستخدم بعيد.",
                    "أسرع في النقل لأنها لا تضغط البيانات."
                ],
                "answer": 0,
                "explanation": "الأداة rsync ذكية حيث تفحص الملفات وتنقل فقط الأجزاء المعدلة (Incremental copy) وتدعم استكمال النقل."
            }
        ]
    },
    "rh124_packages": {
        "easy": [
            {
                "q": "ما هو مدير الحزم الافتراضي والحديث لإدارة وتثبيت البرمجيات في RHEL 9؟",
                "options": [
                    "apt",
                    "yum",
                    "dnf",
                    "rpm"
                ],
                "answer": 2,
                "explanation": "الأداة dnf (Dandified YUM) هي مدير الحزم الافتراضي والحديث في RHEL 9."
            }
        ],
        "medium": [
            {
                "q": "ما هو الفرق بين استخدام dnf و rpm لتثبيت الحزم؟",
                "options": [
                    "dnf يقوم بحل وتحميل الاعتماديات (Dependencies) تلقائياً من الإنترنت، بينما rpm يتطلب تثبيت الاعتماديات يدوياً.",
                    "rpm يقوم بتحميل الاعتماديات بينما dnf لا يفعل.",
                    "لا فرق بينهما فهما نفس الأداة.",
                    "rpm مخصص فقط للـ Root بينما dnf للمستخدم العادي."
                ],
                "answer": 0,
                "explanation": "يتميز dnf بقدرته على سحب كافة التبعيات والاعتماديات وتثبيتها تلقائياً، بينما rpm يتعامل مع ملف الحزمة المفرد فقط ويفشل إن نقصت أي تبعية."
            }
        ],
        "hard": [
            {
                "q": "كيف يمكنك استعلام النظام لمعرفة أي حزمة برمجية قامت بتثبيت ملف محدد بالكامل؟",
                "options": [
                    "rpm -qf /path/to/file",
                    "dnf search /path/to/file",
                    "rpm -qi /path/to/file",
                    "dnf info /path/to/file"
                ],
                "answer": 0,
                "explanation": "الخيار -qf (Query File) في أداة rpm يبحث في قاعدة بيانات النظام لمعرفة الحزمة المالكة لهذا الملف."
            }
        ]
    },
    "rh124_mount": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لربط قرص صلب أو قسم خارجي بمجلد محدد داخل نظام الملفات؟",
                "options": [
                    "connect",
                    "mount",
                    "link",
                    "attach"
                ],
                "answer": 1,
                "explanation": "الأمر mount يقوم بعملية التركيب والربط المباشر للأقراص بمجلد داخل شجرة النظام."
            }
        ],
        "medium": [
            {
                "q": "ما هو الملف المسؤول عن حفظ إعدادات تركيب الأقراص بشكل دائم لتعمل تلقائياً عند إقلاع النظام؟",
                "options": [
                    "/etc/mtab",
                    "/etc/fstab",
                    "/etc/hosts",
                    "/etc/filesystems"
                ],
                "answer": 1,
                "explanation": "الملف /etc/fstab (File System Table) يحتوي على القواعد التي تحدد الأقراص والمسارات والخيارات التي يتم تركيبها تلقائياً عند الإقلاع."
            }
        ],
        "hard": [
            {
                "q": "عند تعديل ملف /etc/fstab، ما هو الأمر الذي يجب تشغيله للتأكد من عدم وجود أخطاء في التكوين قبل إعادة التشغيل؟",
                "options": [
                    "mount -a",
                    "mount -all",
                    "systemctl daemon-reload",
                    "df -h"
                ],
                "answer": 0,
                "explanation": "الأمر mount -a يحاول تركيب كافة المدخلات المسجلة في fstab؛ فإذا كان هناك خطأ سيظهر فوراً، وهو ما يحميك من توقف إقلاع السيرفر."
            }
        ]
    },
    "rh134_scripting": {
        "easy": [
            {
                "q": "ما هو السطر الذي يكتب في بداية سكريبتات Bash لتحديد المفسر الافتراضي لتشغيل الكود (Shebang)؟",
                "options": [
                    "#/bin/bash",
                    "#!/bin/bash",
                    "!/bin/bash",
                    "#!bash"
                ],
                "answer": 1,
                "explanation": "السطر القياسي هو #!/bin/bash ويسمى Shebang وهو يخبر النظام بمكان مفسر الأوامر المطلوب."
            }
        ],
        "medium": [
            {
                "q": "كيف يمكنك تمرير أول مدخل (Parameter) ممرر للسكريبت عند تشغيله داخل الكود؟",
                "options": [
                    "$0",
                    "$1",
                    "$@",
                    "$#"
                ],
                "answer": 1,
                "explanation": "$1 يرمز لأول مدخل ممرر، بينما $0 يرمز لاسم السكريبت نفسه، و $# لعدد المدخلات."
            }
        ],
        "hard": [
            {
                "q": "ما هي الطريقة الصحيحة للتحقق مما إذا كان ملف محدد موجوداً بالفعل داخل شرط if في Bash؟",
                "options": [
                    "if [ -d file.txt ]",
                    "if [ -e file.txt ]",
                    "if [ -f file.txt ]",
                    "الخيار الثاني (أي نوع) أو الثالث (ملف عادي) صحيحان"
                ],
                "answer": 3,
                "explanation": "الخيار -f يتحقق من وجود ملف عادي (Regular file)، والخيارات -e تتحقق من الوجود العام (Exists)."
            }
        ]
    },
    "rh134_scheduling": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لتعديل وكتابة المهام المجدولة الخاصة بالمستخدم الحالي؟",
                "options": [
                    "cron -e",
                    "crontab -e",
                    "crontab -l",
                    "cronjob -write"
                ],
                "answer": 1,
                "explanation": "الأمر crontab -e يفتح محرر النصوص المخصص لكتابة وتعديل المهام المجدولة للمستخدم الحالي."
            }
        ],
        "medium": [
            {
                "q": "ما هي الحقول الخمسة الأولى لجدولة المهام في نظام cron بالترتيب؟",
                "options": [
                    "الدقيقة، الساعة، اليوم من الشهر، الشهر، اليوم من الأسبوع.",
                    "الساعة، الدقيقة، اليوم، الشهر، السنة.",
                    "الدقيقة، الساعة، اليوم، السنة، المستخدم.",
                    "المستخدم، الأمر، الوقت، اليوم، الشهر."
                ],
                "answer": 0,
                "explanation": "الترتيب القياسي لحقول cron الخمسة هو: Minute, Hour, Day of Month, Month, Day of Week."
            }
        ],
        "hard": [
            {
                "q": "ماذا تعني الصيغة التالية لجدولة مهمة في cron: `*/15 2 * * *`؟",
                "options": [
                    "تشغيل المهمة كل 15 دقيقة خلال الساعة 2 صباحاً يومياً.",
                    "تشغيل المهمة يوم 15 في الشهر الساعة 2 صباحاً.",
                    "تشغيل المهمة كل 15 ساعة يومياً.",
                    "تشغيل المهمة يومياً الساعة 2:15 صباحاً."
                ],
                "answer": 0,
                "explanation": "*/15 تعني كل 15 دقيقة، والـ 2 تعني الساعة الثانية صباحاً، والنجوم تعني كل الأيام والشهور."
            }
        ]
    },
    "rh134_tuning": {
        "easy": [
            {
                "q": "ما هو نطاق الأولوية (Nice Values) للعمليات في نظام لينكس؟",
                "options": [
                    "من 0 إلى 100",
                    "من -20 إلى 19",
                    "من -10 إلى 10",
                    "من 1 إلى 10"
                ],
                "answer": 1,
                "explanation": "تبدأ قيمة Nice من -20 (أعلى أولوية ممكنة) إلى 19 (أقل أولوية ممكنة)."
            }
        ],
        "medium": [
            {
                "q": "ما هي أداة الضبط التلقائي لملفات الأداء (Performance Profiles) في Red Hat؟",
                "options": [
                    "tuned",
                    "nice",
                    "renice",
                    "systemctl-tune"
                ],
                "answer": 0,
                "explanation": "الأداة tuned تقوم بضبط أداء العتاد والنواة تلقائياً بناء على ملفات تعريف جاهزة مثل virtual-guest أو throughput-performance."
            }
        ],
        "hard": [
            {
                "q": "إذا كنت مستخدماً عادياً (Non-root user)، ما الذي يمكنك فعله بشأن قيم Nice للعمليات الخاصة بك؟",
                "options": [
                    "يمكنك فقط زيادة قيمة Nice (تقليل الأولوية) للعمليات الخاصة بك، ولا يمكنك خفضها أو تعديل عمليات الآخرين.",
                    "يمكنك زيادة وخفض الأولوية لجميع العمليات.",
                    "لا يمكنك تغيير قيم Nice إطلاقاً.",
                    "يمكنك تعديل أولوية عمليات المستخدمين الآخرين."
                ],
                "answer": 0,
                "explanation": "لحماية موارد النظام، يمنح لينكس المستخدم العادي صلاحية تقليل أولوية عملياته فقط (زيادة قيمة Nice)، بينما يحتاج رفع الأولوية لصلاحيات root."
            }
        ]
    },
    "rh134_acls": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لعرض قوائم التحكم بالوصول المتقدمة (ACLs) لملف معين؟",
                "options": [
                    "ls -l",
                    "getfacl",
                    "showfacl",
                    "setfacl"
                ],
                "answer": 1,
                "explanation": "الأمر getfacl يعرض تفاصيل الصلاحيات المتقدمة والمخصصة لمستخدمين أو مجموعات خارج النطاق الأساسي."
            }
        ],
        "medium": [
            {
                "q": "أي أمر يستخدم لإعطاء صلاحيات قراءة وكتابة مخصصة للمستخدم admin على الملف report.txt باستخدام الـ ACLs؟",
                "options": [
                    "setfacl -m u:admin:rw report.txt",
                    "chmod u+rw admin report.txt",
                    "getfacl -m u:admin:rw report.txt",
                    "setfacl -x u:admin report.txt"
                ],
                "answer": 0,
                "explanation": "الخيار -m يعني تعديل الصلاحية (Modify)، والرمز u:admin:rw يحدد الصلاحيات الخاصة بالمستخدم."
            }
        ],
        "hard": [
            {
                "q": "ماذا تعني علامة الجمع (+) التي تظهر في نهاية صلاحيات الملف عند استعراضه بالأمر `ls -l`؟",
                "options": [
                    "أن الملف يمتلك قواعد صلاحيات متقدمة (ACLs) مطبقة عليه.",
                    "أن الملف يحمل وسم أمان SELinux.",
                    "أن الملف قابل للكتابة من جميع المستخدمين.",
                    "أن الملف يمتلك سمة مخفية (Attribute)."
                ],
                "answer": 0,
                "explanation": "علامة الجمع (+) بعد صلاحيات الملف القياسية تشير لوجود صلاحيات ACLs إضافية للملف."
            }
        ]
    },
    "rh134_selinux": {
        "easy": [
            {
                "q": "ما هو الأمر المستخدم لمعرفة الحالة الحالية لنظام الأمان المطور SELinux؟",
                "options": [
                    "getenforce",
                    "setenforce",
                    "selinux-status",
                    "sestatus"
                ],
                "answer": 0,
                "explanation": "الأمر getenforce يعرض الحالة الحالية بسرعة (Enforcing, Permissive, Disabled)."
            }
        ],
        "medium": [
            {
                "q": "ما هو الفرق بين وضعي Enforcing و Permissive في SELinux؟",
                "options": [
                    "Enforcing يقوم بحظر وتسجيل العمليات المخالفة، بينما Permissive يسجل المخالفات فقط دون حظرها.",
                    "Permissive يقوم بحظر العمليات بينما Enforcing لا يحظر.",
                    "Disabled هو نفسه Permissive.",
                    "Enforcing مخصص فقط للإنترنت و Permissive للشبكة المحلية."
                ],
                "answer": 0,
                "explanation": "في وضع Enforcing يتم تفعيل وتطبيق الحظر الصارم، بينما Permissive يستخدم للاختبار وتتبع الأخطاء حيث يكتفي بكتابة التحذيرات بالسجلات."
            }
        ],
        "hard": [
            {
                "q": "أي جزء في سياق الأمان الخاص بـ SELinux (Context) يعتبر الأهم ويحدد قواعد الوصول والتحكم بالخدمات (Type Enforcement)؟",
                "options": [
                    "المستخدم (User)",
                    "الدور (Role)",
                    "النوع (Type)",
                    "الحساسية (Sensitivity)"
                ],
                "answer": 2,
                "explanation": "حقل النوع (Type) والذي ينتهي باللاحقة _t (مثل httpd_sys_content_t) هو الأهم ويتحكم بجميع قواعد الترخيص والوصول للعمليات."
            },
            {
                "q": "ما هي الطريقة الصحيحة والمعتمدة لتعطيل SELinux بالكامل في نظام RHEL 9؟",
                "options": [
                    "تعديل ملف /etc/selinux/config وتغيير القيمة إلى SELINUX=disabled.",
                    "إضافة وسيطة إقلاع النواة selinux=0 في إعدادات محمل الإقلاع GRUB.",
                    "تشغيل الأمر setenforce 0 في الطرفية كمسؤول.",
                    "حذف حزمة selinux-policy من النظام بالكامل."
                ],
                "answer": 1,
                "explanation": "في RHEL 9، تم إلغاء دعم التعطيل عبر الملف config، والطريقة الصحيحة لتعطيله تماماً هي عبر تمرير المعامل selinux=0 للنواة عند الإقلاع."
            }
        ]
    },
    "rh134_storage": {
        "easy": [
            {
                "q": "ما هو نمط التقسيم الحديث للأقراص الذي يتجاوز حد الـ 2 تيرا بايت ويدعم حتى 128 قسماً أساسياً؟",
                "options": [
                    "MBR",
                    "GPT",
                    "FAT",
                    "LVM"
                ],
                "answer": 1,
                "explanation": "GPT (GUID Partition Table) هو النمط الحديث والبديل لنمط MBR القديم الذي كان محدوداً بـ 2 تيرا بايت و 4 أقسام فقط."
            }
        ],
        "medium": [
            {
                "q": "أي أداة مخصصة لإنشاء وتعديل الأقسام على الأقراص التي تستخدم نظام تقسيم GPT؟",
                "options": [
                    "fdisk",
                    "gdisk",
                    "parted",
                    "الخيار الثاني والثالث صحيحان"
                ],
                "answer": 3,
                "explanation": "الأداة gdisk مخصصة لـ GPT خصيصاً، بينما parted أداة شاملة تدعم كلا النمطين MBR و GPT."
            },
            {
                "q": "ما هي الميزة الأساسية لجدول الأقسام GPT مقارنة بـ MBR؟",
                "options": [
                    "دعم سعة تخزين حتى 2 تيرا بايت كحد أقصى.",
                    "دعم حتى 128 قسماً رئيسياً كحد أقصى والتحقق من سلامة الأقسام عبر CRC checksums.",
                    "أنه يتطلب دائماً محاذاة قطاعات الأقراص يدوياً.",
                    "أنه لا يدعم إقلاع UEFI الحديث."
                ],
                "answer": 1,
                "explanation": "يدعم GPT سعات تخزينية ضخمة جداً، و128 قسماً كحد أقصى، ويوفر حماية وفحصاً لأخطاء البيانات مقارنة بـ MBR الذي يدعم 4 أقسام وسعة 2TB فقط."
            }
        ],
        "hard": [
            {
                "q": "بعد تعديل تقسيم القرص الصلب، ما هو الأمر الذي يجب تشغيله لإجبار النواة على قراءة جدول التقسيم الجديد دون إعادة التشغيل؟",
                "options": [
                    "partprobe",
                    "fdisk -l",
                    "udevadm settle",
                    "vgscan"
                ],
                "answer": 0,
                "explanation": "الأمر partprobe يخبر النواة بإعادة قراءة جدول التقسيم للأقراص وتحديث الأقسام فوراً بالذاكرة."
            }
        ]
    },
    "rh134_lvm": {
        "easy": [
            {
                "q": "ما هي الخطوة الأولى بالترتيب لإنشاء أقسام مرنة باستخدام LVM؟",
                "options": [
                    "PV (Physical Volume) ثم VG (Volume Group) ثم LV (Logical Volume)",
                    "VG ثم PV ثم LV",
                    "LV ثم VG ثم PV",
                    "لا يهم الترتيب"
                ],
                "answer": 0,
                "explanation": "يجب تهيئة القرص أولاً كـ Physical Volume ثم دمجه في Volume Group ثم تقسيم المساحة لـ Logical Volumes."
            }
        ],
        "medium": [
            {
                "q": "أي أمر يستخدم لتوسيع حجم القسم المنطقي LVM بمقدار 5 جيجا بايت إضافية؟",
                "options": [
                    "lvextend -L +5G /dev/vg_name/lv_name",
                    "lvresize -L 5G /dev/vg_name/lv_name",
                    "vgextend -L +5G /dev/vg_name/lv_name",
                    "pvresize -L +5G /dev/vg_name/lv_name"
                ],
                "answer": 0,
                "explanation": "الأمر lvextend مع الخيار -L +5G يضيف مساحة 5 جيجا بايت للقسم المنطقي المذكور."
            }
        ],
        "hard": [
            {
                "q": "بعد تكبير القسم المنطقي LVM، ما هو الأمر المناسب لتوسيع نظام ملفات من نوع XFS ليتطابق مع المساحة الجديدة؟",
                "options": [
                    "resize2fs",
                    "xfs_growfs",
                    "xfs_expand",
                    "mount -o remount"
                ],
                "answer": 1,
                "explanation": "نظام ملفات XFS يتم توسيعه باستخدام الأمر xfs_growfs متبوعاً بمسار مجلد التركيب (Mount point)، بينما resize2fs مخصص لأنظمة ext4."
            }
        ],
        "scenario": [
            {
                "q": "سيناريو: تريد توسيع مساحة قسم البيانات /data المهيأ بنظام ملفات XFS بمقدار 20 جيجا بايت بدون إيقاف تشغيل النظام. ما الخطوات الصحيحة؟",
                "options": [
                    "تشغيل lvextend -L +20G للمجلد المنطقي ثم xfs_growfs على نقطة التركيب.",
                    "تشغيل lvextend -L +20G للمجلد المنطقي ثم resize2fs على نقطة التركيب.",
                    "تقليص الحجم أولاً ثم تشغيل xfs_growfs.",
                    "تشغيل vgextend لإضافة المساحة للقرص الصلب مباشرة."
                ],
                "answer": 0,
                "explanation": "نقوم بتوسيع الحجم المنطقي أولاً بأمر lvextend، ثم نمدد نظام الملفات XFS الحي والنشط دون فصله باستخدام xfs_growfs (حيث أن XFS لا يدعم التقليص)."
            }
        ]
    },
    "rh134_network_storage": {
        "easy": [
            {
                "q": "ما هو البروتوكول القياسي لمشاركة الملفات شبكياً بين أنظمة لينكس المتعددة (Linux to Linux)؟",
                "options": [
                    "SMB",
                    "NFS",
                    "FTP",
                    "SSH"
                ],
                "answer": 1,
                "explanation": "بروتوكول NFS (Network File System) هو المعيار الأساسي والافتراضي لمشاركة المجلدات بين خوادم لينكس شبكياً."
            }
        ],
        "medium": [
            {
                "q": "ما هو البروتوكول القياسي لعمل مشاركة ملفات متوافقة مع أنظمة ويندوز (Linux to Windows)؟",
                "options": [
                    "NFS",
                    "Samba / SMB",
                    "HTTP",
                    "LDAP"
                ],
                "answer": 1,
                "explanation": "تستخدم حزمة Samba بروتوكول SMB (Server Message Block) لمشاركة الملفات والطابعات مع أجهزة ويندوز بسلاسة."
            }
        ],
        "hard": [
            {
                "q": "ما هو الخيار الذي يجب إضافته في سطر تركيب NFS بملف /etc/fstab لضمان عدم توقف إقلاع السيرفر في حال كان خادم المشاركة خارج الخدمة؟",
                "options": [
                    "defaults",
                    "nofail",
                    "sync",
                    "ro"
                ],
                "answer": 1,
                "explanation": "الخيار nofail يمنع السيرفر من الدخول في وضع الطوارئ أو الفشل في الإقلاع في حال عدم القدرة على الوصول للمشاركة الشبكية أثناء التشغيل."
            }
        ]
    },
    "rh134_boot": {
        "easy": [
            {
                "q": "ما هو بارامتر النواة الذي نكتبه في شاشة GRUB2 للدخول في وضع استعادة النظام وكسر الإقلاع؟",
                "options": [
                    "systemd.debug",
                    "rd.break",
                    "init=/bin/bash",
                    "root=/dev/shm"
                ],
                "answer": 1,
                "explanation": "البارامتر rd.break يوقف عملية الإقلاع قبل تسليم التحكم لنظام ملفات الـ Root الافتراضي، مما يمنحك موجه أوامر للإنقاذ."
            }
        ],
        "medium": [
            {
                "q": "عند الدخول في وضع rd.break، أين يقع نظام ملفات القرص الأصلي وما هي صلاحية تركيبه الافتراضية؟",
                "options": [
                    "يقع في المجلد /sysroot بصلاحيات قراءة فقط (Read-only).",
                    "يقع في المجلد /mnt بصلاحيات كاملة.",
                    "يقع في المجلد /root بصلاحيات قراءة فقط.",
                    "يقع في المجلد /sysroot بصلاحيات كتابة وقراءة."
                ],
                "answer": 0,
                "explanation": "النظام الأصلي يتم تركيبه تلقائياً في /sysroot بصلاحية القراءة فقط لحمايته، ويجب إعادة تركيبه بصلاحية كتابة لتعديل كلمة المرور."
            }
        ],
        "hard": [
            {
                "q": "لماذا يجب تشغيل الأمر `touch /.autorelabel` بعد تغيير كلمة مرور root في بيئة الطوارئ؟",
                "options": [
                    "لإجبار نظام SELinux على إعادة تصنيف سياقات الأمان للملفات التي تم تعديلها (مثل ملف shadow) لكي يسمح لك بالدخول عند الإقلاع التالي.",
                    "لتثبيت التعديلات على القرص الصلب.",
                    "لحفظ كلمة المرور في الذاكرة المؤقتة.",
                    "لإعادة تشغيل النظام تلقائياً."
                ],
                "answer": 0,
                "explanation": "بما أن كلمة المرور تم تعديلها خارج النظام الطبيعي، يفقد ملف shadow سياق SELinux الصحيح. هذا الأمر يخبر SELinux بإعادة فحص وتصحيح السياقات عند بدء التشغيل القادم ليسمح لك بالمصادقة والدخول."
            }
        ],
        "scenario": [
            {
                "q": "سيناريو: بعد استعادة كلمة مرور root عبر سياق rd.break، نسيت إنشاء الملف /.autorelabel وأعدت تشغيل النظام. ما الذي سيحدث؟",
                "options": [
                    "سيفتح النظام بشكل طبيعي ولكن سيطلب تغيير كلمة المرور مجدداً.",
                    "سيمنعك نظام SELinux من تسجيل الدخول بكلمة المرور الجديدة بسبب فقدان سياق الحماية لملف shadow.",
                    "سيقوم النظام بعملية autorelabel تلقائياً وتستغرق وقتاً أطول.",
                    "سيتم مسح كلمة مرور root وتصفيرها تلقائياً."
                ],
                "answer": 1,
                "explanation": "الملف /.autorelabel يجبر النظام على إعادة وسم سياقات SELinux لكافة ملفات النظام. بدونه، يفتقد ملف shadow السياق الصحيح ويحظر SELinux الولوج للنظام."
            }
        ]
    },
    "rh134_firewall": {
        "easy": [
            {
                "q": "ما هي الأداة المخصصة لإدارة قواعد جدار الحماية في توزيعات Red Hat عبر سطر الأوامر؟",
                "options": [
                    "iptables",
                    "firewall-cmd",
                    "ufw",
                    "nftables"
                ],
                "answer": 1,
                "explanation": "الأداة firewall-cmd هي الواجهة الرسمية للتحكم بجدار الحماية الذكي firewalld."
            }
        ],
        "medium": [
            {
                "q": "ما هي الطريقة الصحيحة للسماح باتصالات منفذ 80 بشكل دائم لكي لا تختفي القواعد بعد إعادة تشغيل الجدار؟",
                "options": [
                    "firewall-cmd --add-port=80/tcp --permanent",
                    "firewall-cmd --add-port=80/tcp",
                    "firewall-cmd --open-port=80/tcp --save",
                    "firewall-cmd --allow-port=80/tcp"
                ],
                "answer": 0,
                "explanation": "الخيار --permanent يحفظ القاعدة في الإعدادات الدائمة على القرص، ويجب عمل reload للجدار لتفعيلها."
            },
            {
                "q": "كيف يمكن إضافة خدمة الـ HTTP بشكل دائم وجعلها مفعلة بعد إعادة تشغيل جدار الحماية firewalld؟",
                "options": [
                    "firewall-cmd --add-service=http",
                    "firewall-cmd --permanent --add-service=http ثم firewall-cmd --reload",
                    "firewall-cmd --zone=public --add-port=80",
                    "systemctl restart firewalld بعد تعديل الخدمة يدوياً"
                ],
                "answer": 1,
                "explanation": "الخيار --permanent يكتب الإعداد في ملفات التكوين الدائمة على القرص، ويتطلب إعادة التحميل بـ --reload لتطبيقه."
            }
        ],
        "hard": [
            {
                "q": "أي خيار في جدار الحماية يسمح بإنشاء قواعد تصفية غنية ومتقدمة جداً (مثل السماح لـ IP معين فقط بالاتصال بـ منفذ مخصص)؟",
                "options": [
                    "--add-rich-rule",
                    "--add-custom-rule",
                    "--add-policy",
                    "--add-rule-advanced"
                ],
                "answer": 0,
                "explanation": "الخيار --add-rich-rule يتيح صياغة قواعد غنية ومفصلة ودقيقة للغاية للتحكم بالشبكة."
            }
        ]
    },
    "forensics_fs": {
        "easy": [
            {
                "q": "ما هو البرنامج التنفيذي الأساسي في Sleuth Kit لعرض محتويات ملف محذوف بناءً على رقم الـ Inode الخاص به؟",
                "options": [
                    "fls",
                    "icat",
                    "ils",
                    "ffind"
                ],
                "answer": 1,
                "explanation": "الأمر icat يقرأ ويعرض محتوى الملف المروّس برقم inode محدد مباشرة من القرص."
            }
        ],
        "medium": [
            {
                "q": "أي أمر يستخدم لسرد جميع أسماء الملفات والمجلدات (بما فيها المحذوفة) داخل قرص أو قسم مهيأ؟",
                "options": [
                    "icat",
                    "fls",
                    "ils",
                    "mactime"
                ],
                "answer": 1,
                "explanation": "الأمر fls (File List) يسرد أسماء الملفات والأدلة وهياكل المجلدات داخل صورة القرص."
            }
        ],
        "hard": [
            {
                "q": "ما هو حقل جدول الـ Inode في نظام ملفات لينكس؟",
                "options": [
                    "بنية بيانات تخزن جميع خصائص ومعلومات الملف (مثل الحجم، المالك، الصلاحيات، ومواقع البيانات) عدا اسم الملف نفسه.",
                    "الملف الذي يحتوي على كلمات مرور المشرفين.",
                    "المجلد المؤقت لتحميل التعريفات.",
                    "أداة تشفير الملفات في لينكس."
                ],
                "answer": 0,
                "explanation": "الـ Inode هو فهرس الهوية للملف، ويحتوي على كافة بيانات الميتا داتا ومواقع البلوكات على القرص، ولا يتم تخزين اسم الملف فيه (حيث يخزن الاسم في هيكل المجلد نفسه)."
            }
        ]
    },
    "forensics_logs": {
        "easy": [
            {
                "q": "أين يتم تخزين محاولات الولوج وتفاصيل المصادقة واستخدام صلاحيات sudo في توزيعات دبيان وكالي لينكس؟",
                "options": [
                    "/var/log/secure",
                    "/var/log/auth.log",
                    "/var/log/messages",
                    "/var/log/audit.log"
                ],
                "answer": 1,
                "explanation": "في دبيان وكالي تسمى auth.log بينما في ريد هات تسمى secure."
            }
        ],
        "medium": [
            {
                "q": "كيف يمكن للمهاجم أو المسؤول العثور على محاولات الاختراق بـ Brute force لمحاولة كسر كلمة مرور SSH؟",
                "options": [
                    "البحث عن تكرار رسائل 'Failed password' من عناوين IP خارجية في ملف auth.log.",
                    "البحث عن 'Accepted publickey' في ملف syslog.",
                    "قراءة ملف /etc/shadow.",
                    "فحص جدول العمليات النشطة بـ ps."
                ],
                "answer": 0,
                "explanation": "تسجل خدمة sshd محاولات الدخول الفاشلة بعبارة Failed password، وتكرارها السريع يشير لهجوم brute force."
            }
        ],
        "hard": [
            {
                "q": "ما هو دور أداة التنبيه الجنائي `fail2ban` في حماية سيرفر الويب وسيرفر الـ SSH؟",
                "options": [
                    "مراقبة ملفات السجلات تلقائياً وحظر عناوين الـ IP التي تقوم بمحاولات دخول فاشلة متكررة عبر جدار الحماية.",
                    "تشفير كلمات المرور.",
                    "منع المستخدمين من كتابة أوامر معينة.",
                    "أرشفة السجلات وحذفها لتوفير المساحة."
                ],
                "answer": 0,
                "explanation": "أداة fail2ban تقوم بفحص السجلات بشكل مستمر، وعند تكرار الأخطاء تقوم تلقائياً بإضافة وسم حظر للـ IP المخالف في جدار الحماية لفترة محددة."
            }
        ]
    },
    "forensics_memory": {
        "easy": [
            {
                "q": "ما هي الأداة الشهيرة والأكثر استخداماً في التحليل الجنائي للذاكرة العشوائية (RAM Forensics)؟",
                "options": [
                    "Autopsy",
                    "Volatility",
                    "Sleuth Kit",
                    "Wireshark"
                ],
                "answer": 1,
                "explanation": "إطار عمل Volatility هو المعيار القياسي والأقوى عالمياً لتحليل وفحص صور الذاكرة العشوائية RAM."
            }
        ],
        "medium": [
            {
                "q": "أي أمر في Volatility يستخدم لعرض العمليات النشطة التي كانت تعمل في الذاكرة لحظة سحب العينة؟",
                "options": [
                    "linux.pslist",
                    "linux.netscan",
                    "linux.files",
                    "linux.bash"
                ],
                "answer": 0,
                "explanation": "الأمر pslist (أو linux.pslist) يسرد العمليات النشطة المخزنة في الذاكرة العشوائية."
            }
        ],
        "hard": [
            {
                "q": "كيف يساعد فحص الذاكرة العشوائية (RAM Dump) في كشف الهجمات المتقدمة مقارنة بفحص القرص الصلب؟",
                "options": [
                    "يكشف البرمجيات الخبيثة التي تعمل في الذاكرة فقط دون ترك أي ملفات على القرص الصلب (Fileless Malware).",
                    "أسهل في القراءة ولا يتطلب صلاحيات.",
                    "لأن الذاكرة العشوائية لا يمكن مسح بياناتها إطلاقاً.",
                    "لا يوجد فرق حقيقي بينهما."
                ],
                "answer": 0,
                "explanation": "العديد من الهجمات الحديثة تعتمد على تشغيل كود خبيث في الذاكرة مباشرة (Fileless) وتختفي تماماً بمجرد إغلاق الجهاز أو فحص القرص، لذلك يعتبر فحص الذاكرة حاسماً للتحقيق الجنائي."
            }
        ]
    },
    "kernel_internals": {
        "easy": [
            {
                "q": "ما هي البوابة البرمجية التي تستخدمها البرامج في مساحة المستخدم (User Space) لطلب مهام من النواة (Kernel Space)؟",
                "options": [
                    "System Calls",
                    "eBPF",
                    "API",
                    "Signals"
                ],
                "answer": 0,
                "explanation": "استدعاءات النظام (System Calls) مثل sys_open أو fork هي البوابة الوحيدة لطلب العمليات المادية من النواة."
            }
        ],
        "medium": [
            {
                "q": "ما هي التقنية الحديثة التي تسمح بتشغيل برامج معزولة وآمنة (Sandboxed) داخل النواة مباشرة لمراقبة الأمان والأداء دون تعديل كود النواة؟",
                "options": [
                    "eBPF",
                    "Kernel Modules",
                    "Grub",
                    "Docker"
                ],
                "answer": 0,
                "explanation": "تقنية eBPF (Extended Berkeley Packet Filter) ثورية، وتسمح بتشغيل أكواد للمراقبة والتدقيق الأمني مباشرة في النواة بسرعة وأمان كامل."
            }
        ],
        "hard": [
            {
                "q": "أي أداة تستخدم لكتابة سكربتات تتبع حية للـ System Calls باستخدام eBPF على النواة؟",
                "options": [
                    "bpftrace",
                    "strace",
                    "ltrace",
                    "auditctl"
                ],
                "answer": 0,
                "explanation": "الأداة bpftrace تتيح كتابة برامج صغيرة بلغة تتبع خاصة لتتبع الأحداث واستدعاءات النواة لحظياً دون أدنى تأثير على الأداء."
            }
        ]
    }
};
