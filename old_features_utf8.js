...
function injectFeaturesUI() {
    ...
    modal.innerHTML = `
        <div class="search-content">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <span style="color:#8b949e; font-size:0.85rem;"><span class="kbd-hint">Ctrl+K</span> ┘┘╪ز╪ص ╪د┘╪ذ╪ص╪س</span>
                <span style="color:#8b949e; font-size:0.85rem;"><span class="kbd-hint">Esc</span> ┘┘╪ح╪║┘╪د┘é</span>
            </div>
            <input type="text" id="searchInput" class="search-input" placeholder="╪د╪ذ╪ص╪س ╪╣┘ ╪»╪▒╪│╪î ┘à╪╡╪╖┘╪ص╪î ╪ث┘ê ╪ذ╪▒┘ê╪ز┘ê┘â┘ê┘...">
    ...
    if (navButtons) {
        ...
        iconsDiv.innerHTML = `
            <button class="icon-btn" onclick="document.getElementById('searchModal').classList.add('active'); document.getElementById('searchInput').focus();" title="╪ذ╪ص╪س">≡ا¤</button>
            <button class="icon-btn" onclick="toggleTheme()" title="╪ز╪║┘è┘è╪▒ ╪د┘┘à╪╕┘ç╪▒">≡اîô</button>
        `;
...
function initSearch() {
    ...
    input.addEventListener('input', (e) => {
        ...
        if (matches.length === 0) {
            results.innerHTML = '<div style="padding: 10px; color: var(--danger);">┘╪د ╪ز┘ê╪ش╪» ┘╪ز╪د╪خ╪ش.</div>';
            return;
        }
...
function appendCompletionButton(lessonId) {
    ...
    container.innerHTML = `
        <h3 style="color: var(--accent); margin-bottom: 15px;">┘╪│╪ذ╪ر ╪ح┘╪ش╪د╪▓┘â ┘┘è ┘ç╪░╪د ╪د┘╪»╪▒╪│:</h3>
        <div style="display: flex; gap: 20px; flex-wrap: wrap;">
            <label style="cursor: pointer; font-size: 1.1rem; color: ${p.theory ? 'var(--success)' : '#c9d1d9'};">
                <input type="checkbox" ${p.theory ? 'checked' : ''} onchange="toggleLessonMetric('${lessonId}', 'theory')"> ╪د┘┘╪╕╪▒┘è ≡اôû
            </label>
            <label style="cursor: pointer; font-size: 1.1rem; color: ${p.quiz ? 'var(--success)' : '#c9d1d9'};">
                <input type="checkbox" ${p.quiz ? 'checked' : ''} onchange="toggleLessonMetric('${lessonId}', 'quiz')"> ╪د┘╪د╪«╪ز╪ذ╪د╪▒ ظإô
            </label>
            <label style="cursor: pointer; font-size: 1.1rem; color: ${p.lab ? 'var(--success)' : '#c9d1d9'};">
                <input type="checkbox" ${p.lab ? 'checked' : ''} onchange="toggleLessonMetric('${lessonId}', 'lab')"> ╪د┘┘╪د╪ذ ╪د┘╪╣┘à┘┘è ≡اْ╗
            </label>
            <label style="cursor: pointer; font-size: 1.1rem; color: ${p.interview ? 'var(--success)' : '#c9d1d9'};">
                <input type="checkbox" ${p.interview ? 'checked' : ''} onchange="toggleLessonMetric('${lessonId}', 'interview')"> ╪د┘┘à┘é╪د╪ذ┘╪ر ≡اْ╝
            </label>
        </div>
    `;
...
function updateSidebarIcons() {
    ...
    document.querySelectorAll('.lesson-btn').forEach(btn => {
        ...
        if (id) {
            ...
            if (isFullyCompleted) {
                iconSpan.innerText = 'ظءّ ';
                iconSpan.style.color = 'var(--success)';
            } else if (isPartiallyCompleted) {
                iconSpan.innerText = 'ظù ';
                iconSpan.style.color = '#ffb020';
            } else {
                iconSpan.innerText = 'ظء ';
...
function appendEnrichment(lessonId) {
    ...
    const levelNames = {'easy': '≡اات ╪│┘ç┘', 'medium': '≡ااة ┘à╪ز┘ê╪│╪╖', 'hard': '≡ا¤┤ ╪╡╪╣╪ذ', 'scenario': '≡اـ╡ي╕ ╪│┘è┘╪د╪▒┘è┘ê ╪╣┘à┘┘è'};
    let first = true;
                ...
                <div id="quiz-${lessonId}-${level}" class="enrichment-content ${first ? 'active' : ''}">
                    ...
                    ${data[level].map((q, idx) => `
                        <div class="quiz-question" id="quiz-${lessonId}-${level}-q${idx}">
                            <p style="font-weight: bold; margin-bottom: 10px; font-size: 1.1rem;">╪│${idx + 1}: ${q.q}</p>
                            ...
                            <button class="quiz-check-btn" onclick="checkNewQuizAnswer('${lessonId}', '${level}', ${idx}, ${q.correct}, this)" style="margin-top: 10px; padding: 8px 15px; background: var(--accent); color: #fff; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">╪ز╪ص┘é┘é ┘à┘ ╪د┘╪ح╪ش╪د╪ذ╪ر</button>
    ...
    enrichmentDiv.innerHTML = `
        <h2 style="margin-top: 40px; border-bottom: 2px solid var(--border); padding-bottom: 10px;">≡ادب ╪د╪«╪ز╪ذ╪▒ ┘┘ç┘à┘â ┘┘╪»╪▒╪│</h2>
...
window.checkNewQuizAnswer = function(lessonId, level, qIdx, correctIdx, btn) {
    ...
    if (!selected) {
        feedback.innerHTML = '<span style="color:var(--warning)">╪د┘╪▒╪ش╪د╪ة ╪د╪«╪ز┘è╪د╪▒ ╪ح╪ش╪د╪ذ╪ر ╪ث┘ê┘╪د┘ï.</span>';
        return;
    }
    
    if (parseInt(selected.value) === correctIdx) {
        feedback.innerHTML = '<span style="color:var(--success)">ظ£à ╪ح╪ش╪د╪ذ╪ر ╪╡╪ص┘è╪ص╪ر╪î ┘à┘à╪ز╪د╪▓!</span>';
        ...
        feedback.innerHTML = '<span style="color:var(--danger)">ظإî ╪ح╪ش╪د╪ذ╪ر ╪«╪د╪╖╪خ╪ر╪î ╪ص╪د┘ê┘ ┘à╪▒╪ر ╪ث╪«╪▒┘ë.</span>';
    }
};
...
function checkAllQuizzesAnswered(lessonId, level) {
    ...
    if (allHidden && allBtns.length > 0) {
        ...
        if (section && !section.querySelector('.quiz-complete-msg')) {
            ...
            msg.innerHTML = '≡اë ╪ث╪ش╪ذ╪ز ╪╣┘┘ë ╪ش┘à┘è╪╣ ╪د┘╪ث╪│╪خ┘╪ر ╪ذ┘╪ش╪د╪ص!';
...
function initKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        // Arrow Left/Right ظ¤ Navigate between lessons (only if not in input)
...
function appendLessonNavArrows(lessonId) {
    ...
    navDiv.innerHTML = `
        <button class="lesson-nav-btn" ${!prevLesson ? 'disabled' : ''} onclick="${prevLesson ? `renderLesson('${prevLesson.id}')` : ''}">
            ظْ ${prevLesson ? prevLesson.title.substring(0, 30) : '┘╪د ┘è┘ê╪ش╪»'}
        </button>
        <button class="lesson-nav-btn" ${!nextLesson ? 'disabled' : ''} onclick="${nextLesson ? `renderLesson('${nextLesson.id}')` : ''}">
            ${nextLesson ? nextLesson.title.substring(0, 30) : '┘╪د ┘è┘ê╪ش╪»'} ظ
        </button>
    `;
